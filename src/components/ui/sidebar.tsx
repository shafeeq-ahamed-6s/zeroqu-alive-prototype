...
function SidebarProvider({
    ...
    // When hovering over a collapsed sidebar, treat it as expanded for styling
    const state =
        open || (enableHovering && isHovering && !open && !isMobile) ? "expanded" : "collapsed";
...
function Sidebar({
    ...
    const { isMobile, state, openMobile, setOpenMobile, open, setIsHovering, enableHovering } =
        useSidebar();

    const handleMouseEnter = React.useCallback(() => {
        if (!isMobile && collapsible === "icon" && !open && enableHovering) {
            setIsHovering(true);
        }
    }, [isMobile, collapsible, open, setIsHovering, enableHovering]);

    const handleMouseLeave = React.useCallback(() => {
        if (!isMobile && collapsible === "icon" && enableHovering) {
            setIsHovering(false);
        }
    }, [isMobile, collapsible, setIsHovering, enableHovering]);
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
    return (
        <ul
            ...
            className={cn(
                "border-sidebar-border mx-4.5 flex min-w-0 translate-x-px flex-col gap-2 border-l px-2.5 py-0.5",
                "group-data-[collapsible=icon]:hidden",
                className
            )}
            {...props}
